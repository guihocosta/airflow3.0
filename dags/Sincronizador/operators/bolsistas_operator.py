...
class BolsistasOperator(Fapes2ConectaOperator):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.mssql_conecta = None
        self.UUIDS_Novas = None
        self.UUIDS_Novas_Reducao = None
    ...
    def _parse_versao_nivel(self, sigla, nivel_nome, valor=None):
        
        SIGLA_ORIGINAL = sigla.upper()
        sigla = sigla.split('-')[0].upper()
        nivel_nome = nivel_nome.upper()
        
        if valor is None:
            self.log.info("Tentativa de buscar UUID para a sigla '%s' sem um valor associado.", sigla)
            return None

        try:
            valor_formatado = f"{float(valor):.1f}"
        except (ValueError, TypeError):
            self.log.error("Valor '%s' para a sigla '%s' não é um número válido.", valor, sigla)
            return None

        if '%' in nivel_nome:
            self.log.info('Redução')
            niveis_da_sigla = self.UUIDS_Novas_Reducao.get(SIGLA_ORIGINAL)
            if not niveis_da_sigla:
                niveis_da_sigla = self.UUIDS_Novas_Reducao.get(sigla)
        
        else:
            niveis_da_sigla = self.UUIDS_Novas.get(SIGLA_ORIGINAL)
            if not niveis_da_sigla:
                niveis_da_sigla = self.UUIDS_Novas.get(sigla)

        self.log.info("A modalidade da bolsa é ¨%s", niveis_da_sigla)

        if niveis_da_sigla:
            uuid_encontrado = niveis_da_sigla.get(valor_formatado)
                
            if uuid_encontrado:
                self.log.info("UUID encontrado para sigla '%s' e valor '%s'.", sigla, valor)
                return uuid_encontrado
        
        self.log.info(
            "Não foi possível encontrar um UUID correspondente para a sigla '%s' e valor '%s'. "
            "Verifique se este nível existe e está ativo no banco de dados.",
            SIGLA_ORIGINAL, valor_formatado
        )
        return None
    ...
    def _import_dependency(self, data):
        ...
        # Faz o sink da pessoa após todas as execuções
        self.pessoa_operator.sink_importacao()
        return data
    ...
    def _parse_status_name(self, bolsa_situacao):
        match bolsa_situacao:
            case "8": return "ATIVA"
            case "10": return "SUSPENSA"
            case "9": return "FINALIZADA"
            case "12": return "CANCELADA"
            case _: return "EM_AVALIACAO"
    ...
    def execute(self, context):
        ...
        if self.mssql_conecta is None:
            from ..util.sqlserver import SqlServer
            self.mssql_conecta = SqlServer(conn_id="sql_server_conecta")

        if self.UUIDS_Novas is None:
            self.UUIDS_Novas = self._load_versao_nivel_uuids()
            self.UUIDS_Novas_Reducao = {
                k:
                {
                    "{:.1f}".format(float(inner_k) * 0.6): inner_v
                    for inner_k, inner_v in v.items()
                }
                for k, v in self.UUIDS_Novas.items()
            }
